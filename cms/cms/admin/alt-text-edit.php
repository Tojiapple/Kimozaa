...
require __DIR__ . '/../includes/validate.php'; // Fix path to validate.php
...
if ($id) {                                                // If valid article id
    $sql = "SELECT i.image_id, i.file, i.alt 
            FROM image   AS i
            JOIN article AS a
              ON i.image_id = a.image_id
           WHERE a.article_id = :id;";                            // SQL to get image data
    $image = pdo($pdo, $sql, ['id' => $id])->fetch();     // Get image data (fix parameter binding and column names)
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {               // If form was submitted
    ...
    if ($errors['alt']) {                                   // If alt text not long enough
        ...
        $sql = "UPDATE image 
                   SET alt = :alt 
                 WHERE image_id = :image_id;";                          // SQL to update image table
        pdo($pdo, $sql, $updateData);                       // Update alt text
