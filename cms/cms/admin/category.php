...
require __DIR__ . 'cms/includes/validate.php';    
...
if ($id) {                                                // If got an id
    $sql = "SELECT category_id, name, description, navigation 
              FROM category 
             WHERE category_id = :id;";                            // SQL statement
    $category = pdo($pdo, $sql, ['id' => $id])->fetch();          // Get category data
    ...
    if ($invalid) {                                       // If data is invalid
        ...
        if ($id) {                                        // If there is an id
            $sql = "UPDATE category 
                     ...
                     WHERE category_id = :id;";                    // SQL to update category
